...
rm -fr infer-out/ infinite.o *~ infer-run-mbedtls.log

# executing the saved github tree
make clean
bear -- make -j30
time /huge/jvanegue/PUBLIC_GITHUB/infer/infer/bin/infer run --pulse-only --compilation-database compile_commands.json 2> infer-run-mbedtls.log
