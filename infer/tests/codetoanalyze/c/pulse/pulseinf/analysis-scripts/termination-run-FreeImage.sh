...
rm -fr infer-out/ infinite.o *~ infer-run-freeimage.log

# executing the saved github tree
make clean
time /huge/jvanegue/PUBLIC_GITHUB/infer/infer/bin/infer run --debug-level=0 --pulse-only -- make -j30 2> infer-run-freeimage.log
