...
rm -fr infer-out/ infinite.o *~ infer-run-proftpd.log

make clean
./configure
bear -- make -j30
time /huge/jvanegue/PUBLIC_GITHUB/infer/infer/bin/infer --pulse-only --compilation-database compile_commands.json 2> infer-run-proftpd.log
