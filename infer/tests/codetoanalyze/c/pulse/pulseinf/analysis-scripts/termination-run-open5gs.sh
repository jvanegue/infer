...
rm -fr infer-out/ infinite.o *~ infer-run-open5gs.log

# executing the saved github tree
#bear -- ninja -C build
time /huge/jvanegue/PUBLIC_GITHUB/infer/infer/bin/infer run --debug-level 0 --pulse-only --compilation-database compile_commands.json --keep-going 2> infer-run-open5gs.log
