...
rm -fr infer-out/ infinite.o *~ infer-run-openssl.log

bear -- make -j30
time /huge/jvanegue/PUBLIC_GITHUB/infer/infer/bin/infer --pulse-only --pulse-widen-threshold=5 --compilation-database compile_commands.json 2> infer-run-openssl.log

# executing the saved github tree
#make clean
#time /huge/jvanegue/PUBLIC_GITHUB/infer/infer/bin/infer run --pulse-only --pulse-widen-threshold 3 -- make -j30 2> infer-run-openssl.log
