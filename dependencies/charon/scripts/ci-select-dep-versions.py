...
if ci_event in ["pull_request", "merge_group"]:
    if ci_event == "pull_request":
        ...
        pr_commit = subprocess.check_output(["git", "rev-parse", "HEAD^2"])
        pr_commit = pr_commit.decode("utf-8").strip()
        prs = json.loads(
            subprocess.check_output(["gh", "pr", "list", "--json", "number,headRefOid"])
        )
        pr_number = None
        for pr in prs:
    ...
    for line in description.splitlines():
        if line.startswith("ci:"):
            r = re.match(
                "^ci: use https://github.com/[A-Za-z]*/([A-Za-z]*)/pull/([0-9]*)", line
            )
            if r is None:
...
for project, ref in project_refs.items():
    if project not in DEPENDENT_PROJECTS:
        eprint(
            f"ERROR: repo `{project}` is not a dependent project. Accepted values are:",
